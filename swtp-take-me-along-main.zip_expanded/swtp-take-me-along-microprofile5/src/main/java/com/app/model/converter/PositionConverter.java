    ...
    public Position convertToEntityAttribute(final byte[] dbData) {
        ...
        if (dbData.length < 21) {
            throw new RuntimeException(
                    "convertToEntityAttribute failed: size < 21 bytes, actual size: " + dbData.length);
        }
        ...
        if (byteOrder == 0x00) {
            ...
            throw new RuntimeException(
                    "convertToEntityAttribute failed: unknown value for byteOrder, actual value: " + byteOrder);
        }

        final int geometryType = buffer.getInt();
        if (geometryType != 1) {
            throw new RuntimeException(
                    "convertToEntityAttribute failed: geometry type != 1 (not a point), actual value: " + geometryType);
        }
